"use client";
import { DEFAULT_API_HOST, REQUEST_TIMEOUT_MS } from "@/app/constant";
...
export class HunyuanApi implements LLMApi {
  path(): string {
    ...
    if (baseUrl.length === 0) {
      baseUrl = DEFAULT_API_HOST + "/api/proxy/tencent";
    }

    if (baseUrl.endsWith("/")) {
      baseUrl = baseUrl.slice(0, baseUrl.length - 1);
    }
    if (!baseUrl.startsWith("http") && !baseUrl.startsWith("/api/tencent")) {
