const AFTER_CHAT_CACHE = "after-chat-cache";
const AFTER_CHAT_FILE_CACHE = "after-chat-file";
...
self.addEventListener("install", function (event) {
  ...
  event.waitUntil(
    caches.open(AFTER_CHAT_CACHE).then(function (cache) {
      return cache.addAll([]);
    }),
  );
});
...
async function upload(request, url) {
  ...
  const cache = await caches.open(AFTER_CHAT_FILE_CACHE)
